...
public final class STSD extends BOX {
    ...
    public static class AudioSampleDescription {
        ...
        public void read(DataInputStream in) throws IOException {
            ...
            byte[] b = new byte[4];
            in.read(b);
            sampleRate = BufferUtils.from16Bit(b);
            if (innerVersion != 0) {
    ...
    public static VideoSampleDescription getVideoSampleDescription(
            SampleDescription desc) throws IOException {
        VideoSampleDescription sd = new VideoSampleDescription();
        DataInputStream bin = new DataInputStream(new ByteArrayInputStream(
                desc.description));
        sd.read(bin);
        return sd;
    }

    public static AudioSampleDescription getAudioSampleDescription(
            SampleDescription desc) throws IOException {
