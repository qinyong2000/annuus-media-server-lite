...
public class Amf3Serializer {
    ...
    private void writeAmf3Int(int value) throws IOException {
        // Sign contraction - the high order bit of the resulting value must
        // match every bit removed from the number
        // Clear 3 bits
        value &= 0x1fffffff;
        if (value < 0x80) {
            out.writeByte(value);
    ...
    private void writeAmf3String(String s) throws IOException {
        if (s.length() == 0) {
            ...
            for (int i = 0, len = stringRefTable.size(); i < len; i++) {
    ...
    private void writeAmf3Object(AmfValue amfValue) throws IOException {
        for (int i = 0, len = objectRefTable.size(); i < len; i++) {
    ...
    public void write(AmfValue amfValue) throws IOException {
        ...
        case AmfValue.AMF_INT:
            ...
            if (v >= -0x10000000 && v <= 0xFFFFFFF) { // check valid range for
                                                      // 29bits
                out.writeByte(0x04);
                writeAmf3Int(v);
            } else {
